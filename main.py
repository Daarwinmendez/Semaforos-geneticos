...
def main():
    escenario = "Escenarios/1. Zona Universitaria (UASD)/osm.sumocfg"
    n_pobladores = 4
    n_generaciones = 2
    n_elite = 2
    prob_mutacion = 0.7

    # === GENERAR POBLACIÓN INICIAL (con un path de configuración base cualquiera, por ejemplo extraído antes) ===
    path_base_conf = "configs/Semaforos/base"
    if not os.path.exists(path_base_conf):
        os.makedirs(path_base_conf)
        print(f"La carpeta base {path_base_conf} debe tener archivos .json de semáforos extraídos del escenario.")

    poblacion = generar_poblacion_inicial(path_base_conf, n_pobladores=n_pobladores)
    benchmark_fitness = []
    mejores_cromosomas = []
    ...
    for gen in range(n_generaciones):
        ...
        for i, cromosoma in enumerate(poblacion):
            ...
            # --------- CONFIGURACIÓN DINÁMICA POR INDIVIDUO ---------
            carpeta_cromo_conf = f"configs/Semaforos/gen{gen+1}_ind{i+1}"
            aplicar_cromosoma_a_jsons(cromosoma, carpeta_cromo_conf)

            # --------- CARPETA DE RESULTADOS DINÁMICA ---------
            carpeta_resultados = f"resultados/gen{gen+1}_ind{i+1}"
            os.makedirs(carpeta_resultados, exist_ok=True)

            # --------- CORRER SIMULACIÓN USANDO ESA CONFIG ---------
            ...
            print(f"Fitness cromosoma {i+1}: {fitness}")
    ...
    carpeta_mejor_conf = "configs/Semaforos/mejor_global"
    carpeta_resultados_final = "resultados/mejor_global"
